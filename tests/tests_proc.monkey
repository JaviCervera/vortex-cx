...
Public
	Method OnCreate%()
		...
		'Load font
		font = Font_Cache("eurofurence_24.fnt.xml")
	...
	Method OnUpdate%()
		...
		Viewer_SetPerspective(viewer, 45, Float(DeviceWidth()) / DeviceHeight(), 0.001, 1000)
	...
	Method OnRender%()
		...
		Painter_SetColor(0, 1, 0)
		Local text$ = "<Press space key or touch screen to change test>"
		Font_Draw(font, (DeviceWidth() - Font_GetTextWidth(font, text))/2, DeviceHeight() - Font_GetTextHeight(font, text), text)
		Painter_SetColor(1, 0, 0)
		text = GetFPS() + " FPS"
		Font_Draw(font, 0, DeviceHeight() - Font_GetTextHeight(font, text), text)
		
		If currentTest = TEST_ANIMATION
			Painter_SetColor(0, 0, 0)
			Font_Draw(font, 4, 4, "Frame: " + currentFrame)
		End
